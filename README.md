...
> **MindCheck** | Machine Learning + Full-Stack Web Application  
...
CAPSTONE/
│
├── Mindcheck.ipynb    # Main ML notebook (EDA → training → export)
...
> **Note:** The trained models live in `files/models/`. They are generated by running `MindCheck.ipynb` end-to-end (requires a working Python environment with Jupyter and the above packages, plus `matplotlib`, `seaborn`).
...
Open and run **`MindCheck.ipynb`** cell by cell (or *Run All*) in Jupyter Notebook / JupyterLab. The notebook will:
